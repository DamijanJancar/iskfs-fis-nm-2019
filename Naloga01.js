...
socket.on("ukazArduinu", function(štUkaza) {
        ...
        if (štUkaza == "5") {
            board.digitalWrite(12, board.HIGH); // zapišemo +5V na pin 12
        }
        if (štUkaza == "4") {
            board.digitalWrite(12, board.LOW); // zapišemo 0V na pin12
        }
        if (štUkaza == "7") {
            board.digitalWrite(9, board.HIGH); // zapišemo +5V na pin 9
        }
        if (štUkaza == "6") {
            board.digitalWrite(9, board.LOW); // zapišemo 0V na pin 9
        }
        if (štUkaza == "9") {
            board.digitalWrite(8, board.HIGH);
            board.digitalWrite(9, board.HIGH);
            board.digitalWrite(12, board.HIGH);
            board.digitalWrite(13, board.HIGH);// zapišemo 5V na pin vse pine
        }
        if (štUkaza == "8") {
            board.digitalWrite(8, board.LOW);
            board.digitalWrite(9, board.LOW);
            board.digitalWrite(12, board.LOW);
            board.digitalWrite(13, board.LOW);// zapišemo 5V na pin vse pine
        }
    });
